...
static void btflded(argc,argv)
  int argc;
  char **argv;
{
  if (!mio) {
    dispscrn("btflded.scr",0);
...
static int getfldtype(fld,data,opt,len)
  int fld,len;
  char *data,*opt;
{
  int i;
...
static int putfldtype(fld,data,opt,len)
  int fld,len;
  char *data,*opt;
{
  int i;
